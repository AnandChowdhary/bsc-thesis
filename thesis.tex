...
\paragraph{S3} Although there are no binary files required for the API, a database is not ideal for storing larger amounts of text data. For storing these items like raw incoming emails (typically a few kilobytes in size), Amazon Web Services (AWS)'s Simple Storage Service (S3) is used. S3 provides object storage along with backup, recovery, and encryption.
