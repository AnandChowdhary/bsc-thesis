...
\subsubsection{Uptime Monitoring}
...
To monitor the uptime of the services and get real-time notifications in case of downtime, \emph{Uptime Robot} is installed []. In intervals of 5 minutes, Uptime Robot makes network requests to the frontend and backend URLs and sends an email and Slack notification in case any URL is available.

Over the course of testing weeks, no downtime was recorded for the backend, but the frontend was unavailable for a period of 2 minutes and 57 seconds on June 7 because of a Netlify deploying error, which was immediately fixed because of the real-time notifications.
...
Licensed under Apache 2, Sentry is open source but offers a hosted service with a generous free plan, which is used for this project. The @sentry/browser package is used to track errors originating in client-side web applications running in browsers, and @sentry/node is the Node.js package.

Over the course of testing weeks, 3 errors were tracked on the frontend project, but no errors were found in the backend project.
